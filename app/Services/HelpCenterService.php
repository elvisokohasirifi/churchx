...
class HelpCenterService
{
    ...
    private function articles(): array
    {
        return [
            ...
            $this->article(
                ...
                'Member is a person in the church directory who may have a branch leader and may separately have a shepherd for personal pastoral oversight. A member designated as a shepherd is supervised by their active branch leader when one is assigned. MemberBranch records a person’s branch history and identifies the current primary branch.',
                [
                    'Member' => 'A church member profile containing identity, contact, demographic, membership, and optional branch-leader and shepherd information.',
                    'MemberBranch' => 'The historical assignment of a member to a branch, including dates, primary status, and transfer state.',
                ],
                ['primary_branch_id', 'membership_number', 'branch_leader_id', 'shepherd_id', 'is_shepherd', 'first_name', 'middle_name', 'last_name', 'phone', 'alternative_phone', 'email', 'address', 'date_of_birth', 'gender', 'marital_status', 'occupation', 'highest_education', 'date_joined', 'membership_status', 'notes'],
                ...
                [
                    ...
                    $this->task('Import members from CSV', ['Open People → Members and select Import CSV.', 'Use the full sample template, or upload one of the supported compact membership formats.', 'For a compact file, choose the default branch; location is saved as the member address.', 'Upload the completed file. Branch leader is optional, and no members are created if any row fails validation.'], 'members.create'),
                    $this->task('Filter members', ['Open Reports → Filter Members.', 'Choose one accessible branch or leave the branch filter set to all accessible branches.', 'Select a member field and operator, then enter the condition when the operator requires one.', 'Apply the filters to review the matching members. Use commas for list and range operators.']),
    ...
    private function fieldDefinition(string $field): string
    {
        return [
            ...
            'branch_leader_id' => 'The optional active branch-leader appointment responsible for overseeing the member. It defaults to the branch’s first active leader when available and may be left blank. When selected, it must be different from the shepherd.',
