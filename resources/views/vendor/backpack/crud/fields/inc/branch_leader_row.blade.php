<div class="card mb-0" data-branch-leader-row>
    <div class="card-body">
        @if (! empty($leader['id']))<input type="hidden" name="leaders[{{ $index }}][id]" value="{{ $leader['id'] }}">@endif
        <div class="row g-3 align-items-end">
            <div class="col-md-4">
                <label class="form-label">Member</label>
                <select class="form-select" name="leaders[{{ $index }}][member_id]" required>
                    <option value="">Select member</option>
                    @foreach ($field['members'] as $memberId => $memberName)<option value="{{ $memberId }}" @selected(($leader['member_id'] ?? null) === $memberId)>{{ $memberName }}</option>@endforeach
                </select>
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-1 text-end"><button type="button" class="btn btn-outline-danger" data-remove-branch-leader title="Remove leader"><i class="la la-trash"></i></button></div>
            <div class="col-12">
